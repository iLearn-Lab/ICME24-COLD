...
class TVR(Dataset):
    vocab = torchtext.vocab.pretrained_aliases['glove.840B.300d'](
        cache='/root/zhangshuaike/ACRM/proposal_free/data/TMLGA/')
    ...
    def __init__(self, features_path,
        ...
        for i in range(size):
            my_dict = self.dataset[i]
            duration = my_dict['duration']
            vid = my_dict['video']
            if vid not in existed_fea:
                continue
            time_start = my_dict['time_start']
            time_end = my_dict['time_end']
            sentence = my_dict['discription']
            if (time_start < time_end):
                anno_pairs.append(
                    {
                        'video': vid,
                        'duration': duration,
                        'times': [max(time_start, 0), min(time_end, duration)],
                        'description': sentence,
                    }
                )
    ...
    def __getitem__(self, index):
        ...
        return index, i3dfeat, tokens, torch.from_numpy(loc_start), torch.from_numpy(loc_end), torch.from_numpy(
            localization), \
