...
def trainer(cfg):
    ...
    if cfg.MODE_TRAIN == 'resume':
        model_state_dict = torch.load("./checkpoints/{}/24.67.pkl".format(cfg.EXPERIMENT_NAME, cfg.MODEL_NAME))
        model.load_state_dict(model_state_dict)

    # model = torch.load("./checkpoints/{}/model_{}.txt".format(cfg.EXPERIMENT_NAME, cfg.MODEL_NAME))
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        for iteration, batch in enumerate(dataloader_train):
            ...
            batch_size, seq_len, _ = videoFeat.shape
            h_labels = torch.zeros(batch_size, seq_len).cuda()
            for i in range(batch_size):
                h_labels[i][frame_start[i]: frame_end[i]] = 1
            ...
            duration = batch[15]
            vid_names = batch[16]
            loss, individual_loss, pred_start, pred_end, attention, atten_loss = model(videoFeat, videoFeat_lengths,
                                                                                       tokens, tokens_lengths, start,
                                                                                       end, localiz, frame_start,
                                                                                       frame_end, h_labels)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for iteration, batch in enumerate(dataloader_test):
                ...
                duration = batch[15]
                vid_names = batch[16]
                loss, individual_loss, pred_start, pred_end, attention,atten_loss = model(videoFeat, videoFeat_lengths, tokens, tokens_lengths, start, end, localiz,frame_start,frame_end,h_labels)
        ...
        a, r1l7 = vis_test.plot(epoch)
        if r1l7 >= best_r1i7:
            best_r1i7 = r1l7
            torch.save(model.state_dict(), '{}{}{}{}.pkl'.format('./checkpoints/', cfg.EXPERIMENT_NAME, '/', str(best_r1i7)))
