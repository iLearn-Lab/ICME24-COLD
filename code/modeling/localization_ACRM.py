...
from modeling.dynamic_filters.informationBottleneck import Encoder, MIEstimator
...
class Localization_ACRM(nn.Module):
    def __init__(self, cfg):
        ...
        # video and query information bottleneck encode
        self.video_encoding = Encoder(dim=cfg.INFORMATION_BOTTLENECK.DIM, z_dim=cfg.INFORMATION_BOTTLENECK.LATENT_SIZE, drop_rate=cfg.INFORMATION_BOTTLENECK.DROPOUT)
        self.query_encoding = Encoder(dim=cfg.INFORMATION_BOTTLENECK.DIM, z_dim=cfg.INFORMATION_BOTTLENECK.LATENT_SIZE, drop_rate=cfg.INFORMATION_BOTTLENECK.DROPOUT)
        ...
        # fusion information bottleneck encode
        self.features_encoding = Encoder(dim=cfg.INFORMATION_BOTTLENECK.FUSION_DIM, z_dim=cfg.INFORMATION_BOTTLENECK.FUSION_LATENT_SIZE, drop_rate=cfg.INFORMATION_BOTTLENECK.DROPOUT)
        ...
        # compute cross loss
        self.mi_estimator = MIEstimator(dim=cfg.INFORMATION_BOTTLENECK.LATENT_SIZE)
    ...
    def forward(self, videoFeat, videoFeat_lengths, tokens, tokens_lengths, start, end, localiz, frame_start, frame_end, h_labels):
        ...
        v_means, v_std, output_video = self.video_encoding(output_video)
        q_means, q_std, filter_start = self.query_encoding(filter_start)
        ...
        means, std, output = self.features_encoding(output)
        ...
        # cross model information bottleneck loss
        print()
        cross_loss = self.mi_estimator(v_means, v_std, output_video, q_means, q_std, filter_start, h_labels)

        # fusion information bottleneck loss
        var = std.pow(2)
        kl_loss = - 0.5 * (1 + var.log() - means.pow(2) - var).mean()
        ...
        if True:
            ...
            total_loss = start_loss + end_loss + 1*inter_loss + cross_loss + 0.1 * kl_loss# 2 * cross_loss
        else:
