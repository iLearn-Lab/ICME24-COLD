...
def build_dataset(dataset_name, dataset_catalog, cfg):
    ...
    args = data["args"]
    if (cfg.DATASETS.TRAIN == "anet_cap_train" or cfg.DATASETS.TRAIN == "tacos_train" or cfg.DATASETS.TRAIN == "tvr_train") and cfg.FEATURE_TYPE == 'i3d':
        if cfg.DATASETS.TRAIN == "anet_cap_train" or cfg.DATASETS.TRAIN == "tacos_train":
            args['features_path'] = args['features_path'].split('.')[0][:-3] + 'i3d.hdf5'
        else:
            args['features_path'] = args['features_path'].split('.')[0][:-3] + 'i3d.h5'
